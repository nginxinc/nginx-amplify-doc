...
For manual installation, please check the [user guide]({{< relref "/nginx-amplify-agent/install-configure-amp-agent/installing-agent#installing-the-agent-manually" >}}).
...
1. The Amplify Agent package has been successfully [installed]({{< relref "/nginx-amplify-agent/install-configure-amp-agent/installing-agent.md" >}}), and no warnings were shown during the installation.

2. The `amplify-agent` process is running and updating its [log file]({{< relref "/nginx-amplify-agent/install-configure-amp-agent/configuring-agent#agent-logfile" >}}).
...
12. Outbound TLS/SSL from the system to *receiver.amplify.nginx.com* is not restricted. This can be checked with *curl(1)*. [Configure a proxy server]({{< relref "/nginx-amplify-agent/install-configure-amp-agent/configuring-agent#setting-up-a-proxy" >}}) for the agent if required.
...
Guide to [uninstall Amplify Agent]({{< relref "/nginx-amplify-agent/install-configure-amp-agent/uninstalling-agent" >}})
...
Refer to the [Configuring the Agent]({{< relref "/nginx-amplify-agent/install-configure-amp-agent/configuring-agent#overriding-the-effective-user-id" >}}) section in the documentation.
